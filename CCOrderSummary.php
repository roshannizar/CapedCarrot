...
<div class="main" id="main" onmouseover="closebtn()">
    ...
    <div>
        <div class="maincontainer">
            ...
            <center>
                ...
                <div class="activator">Payment Option</div>
            ...
            <div class=" fade" style="margin-left:50px;margin-top:30px">
                ...
                $DataGridView="SELECT * FROM cart c,stock s where c.username='$user' AND c.stockid=s.stockid";
                $orderRemove="SELECT * FROM order_detail";
                $stockdetail="SELECT price FROM stock s,cart c where c.stockid=s.stockid";
                $stock=mysql_query($DataGridView);
                $order=mysql_query($orderRemove);
                $stockall=mysql_query($stockdetail);
                while($row2=mysql_fetch_array($stockall))
                {
                }
                while($row1=mysql_fetch_array($order))
                {
                }
                if($DataGridView=="" || $DataGridView=="" || $DataGridView==null)
                {
                ...
                while($row=mysql_fetch_array($stock))
                {
                    ...
                    <table cellspacing="10"style="width:100%;">
                        ...
                        <tr>
                            <td>
                                <form method="POST" action="CCOrderSummary.php">
                                    ...
                                    <input type="text" placeholder="Enter Number of Heads" name="noheads" value="<?php echo $row["cquantity"] ?>">
                                    <input type="date" placeholder="Pick a Date" name="orderdate" value="<?php echo $row1["date"]; ?>"><br/>
                                    <input type="text" hidden name="ordertype" value="Pending">
                                    <input type="text" hidden name="orderid" value="<?php echo $row1["orderid"];?>">
                                    <input type="text" hidden name="cartid" value="<?php echo $row["cartid"]; ?>">
                                    <input type="text" hidden name="user" value="<?php session_start(); echo $_SESSION["loginuser"]; ?>">
                                    <input type="submit" value="Remove Item" name="btnRemove" class="addcart">
                                </form>
                            </td>
                        </tr>
                    </table>
    ...
    <center>
        <br/>
        <label>Next Make your Payment</label>
        <a href="CCOrderPayment.php"><button style="outline:none;background-color:teal;color:white;font-size:18px;border:1px solid teal;margin-left:10px;border-radius:5px;padding:5px;padding-left:10px;padding-right:10px;cursor:pointer">Pay</button></a>
    </center>
    <br/><br/>
</div>
<footer>
